...
<div class="min-h-screen bg-gray-50 flex flex-col items-center p-4 sm:px-6 lg:px-8">
    ...
    <div class="w-full bg-white rounded-3xl shadow-xl overflow-hidden animate-fade-in
        ...
        <div class="relative p-8 md:p-10 lg:p-12">
            ...
            {{-- Section Informations Personnelles Actuelles --}}
                    ...
                    <ul class="list-disc list-inside text-gray-700 space-y-2">
                        @foreach($user->notes as $note)
                            <li class="border-b border-gray-100 pb-2 last:border-b-0">
                                {{ $note->valeur ?? $note->contenu }} {{-- Use 'valeur' if it's the correct field --}}
            ...
            <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200 mb-8 animate-fade-in">
                ...
                {{-- FIX: Changed $user->fichiers to $user->fichiersPossedes --}}
                ...
                @else
                    <ul class="list-disc list-inside text-gray-700 space-y-2">
                        ...
                        @foreach($user->fichiersPossedes as $fichier)
                            <li class="border-b border-gray-100 pb-2 last:border-b-0 flex justify-between items-center">
                                {{-- Assurez-vous que votre modèle Fichier a bien un champ 'chemin' --}}
                                <a href="{{ asset('storage/' . $fichier->chemin) }}" target="_blank" class="text-blue-600 hover:underline inline-flex items-center">
                                    <i class="fas fa-paperclip mr-2 text-blue-400"></i> {{ $fichier->titre ?? $fichier->nom_fichier }} {{-- Utiliser titre ou nom_fichier --}}
                                </a>
            ...
            <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-fade-in">
                ...
                <form method="POST" action="{{ route('stagiaires.profiles.update', $user->id) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="cin" id="cin" class="form-input @error('cin') is-invalid @enderror" value="{{ old('cin', $user->cin) }}">
                        ...
                        <div>
                            <label for="universite" class="block text-sm font-medium text-gray-700 mb-1">Université :</label>
                            <input type="text" name="universite" id="universite" class="form-input @error('universite') is-invalid @enderror" value="{{ old('universite', $user->universite) }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="faculte" id="faculte" class="form-input @error('faculte') is-invalid @enderror" value="{{ old('faculte', $user->faculte) }}">
                            @error('faculte') <div class="text-red-500 text-xs mt-1">{{ $message }}</div> @enderror
                        </div>
                        <div class="md:col-span-2">
                            <label for="titre_formation" class="block text-sm font-medium text-gray-700 mb-1">Titre de formation :</label>
                            <input type="text" name="titre_formation" id="titre_formation" class="form-input @error('titre_formation') is-invalid @enderror" value="{{ old('titre_formation', $user->titre_formation) }}">
                    ...
                    <div class="flex justify-center mt-8">
                        <button type="submit" class="py-3 px-6 rounded-lg text-lg font-semibold text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out transform hover:scale-105 active:scale-95">
                            <i class="fas fa-save mr-3"></i> Mettre à jour mon profil
                        </button>
                    </div>
                </form>
            </div>
...
{{-- Pas de JavaScript spécifique nécessaire pour cette page, car il n'y a pas de modale ou de logique interactive complexe. --}}
